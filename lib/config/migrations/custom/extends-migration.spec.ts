...
describe('config/migrations/custom/extends-migration', () => {
  it('it migrates preset strings to array', () => {
    expect(ExtendsMigration).toMigrate({ extends: ':js-app' } as any, {
      extends: ['config:js-app'],
    });

    expect(ExtendsMigration).toMigrate({ extends: 'foo' } as any, {
      extends: ['foo'],
    });
  });
  ...
  it('should remove non string values', () => {
    expect(ExtendsMigration).toMigrate({ extends: [{}] } as any, {
      extends: [],
    });
  });
