...
hostRules.find = jest.fn((_) => ({ token: 'abc' }));
...
describe('workers/repository/update/branch/lock-files/index', () => {
  ...
  describe('getAdditionalFiles', () => {
    beforeEach(() => {
      ...
      jest
        .spyOn(npm, 'generateLockFile')
        .mockResolvedValueOnce({ lockFile: 'some lock file contents' });
      jest
        .spyOn(yarn, 'generateLockFile')
        .mockResolvedValueOnce({ lockFile: 'some lock file contents' });
      jest
        .spyOn(pnpm, 'generateLockFile')
        .mockResolvedValueOnce({ lockFile: 'some lock file contents' });
